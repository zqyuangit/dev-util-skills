...
description: "Use when explicitly invoked as openai-api-docs or when the canonical openai-docs skill is unavailable and a legacy local OpenAI API documentation workflow is needed."
---


# OpenAI API Docs

This is the legacy local OpenAI API documentation helper. Prefer the canonical `openai-docs` skill for normal OpenAI product, API, Codex, model-selection, model-upgrade, and prompt-upgrade requests.

Use this skill only when it is explicitly invoked or when the canonical skill is unavailable. Provide authoritative, current guidance from OpenAI developer docs using the developers.openai.com MCP server. Always prioritize the developer docs MCP tools over web.run for OpenAI-related questions. Only if the MCP server is installed and returns no meaningful results should you fall back to web search.
